...
class CAE(nn.Module):
    def __init__(self, in_channels = 3, rep_dim = 256):
        ...
        self.enc_bn1_F = nn.BatchNorm2d(num_features=nf, affine=False)
        ...
        self.enc_bn2_F = nn.BatchNorm2d(num_features=nf * 2, affine=False)
        ...
        self.enc_bn3_F = nn.BatchNorm2d(num_features=nf * 4, affine=False)
    ...
    def encode(self, x, update_batch_stats=True):
        if not update_batch_stats:
            x = self.enc_conv1(x)
            x = self.enc_bn1_F(x)*self.enc_bn1.weight[None,:,None,None]+self.enc_bn1.bias[None,:,None,None]
            x = self.enc_act1(x)

            x = self.enc_conv2(x)
            x = self.enc_bn2_F(x)*self.enc_bn2.weight[None,:,None,None]+self.enc_bn2.bias[None,:,None,None]
            x = self.enc_act2(x)

            x = self.enc_conv3(x)
            x = self.enc_bn3_F(x)*self.enc_bn3.weight[None,:,None,None]+self.enc_bn3.bias[None,:,None,None]
            x = self.enc_act3(x)
